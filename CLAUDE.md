...
- `kikusan/download.py`: Core download logic with unavailable video protection
  ...
  - **Browser impersonation**: `_impersonate_opts()` in `download.py` sets yt-dlp's `impersonate` opt (a `yt_dlp.networking.impersonate.ImpersonateTarget`, built via `ImpersonateTarget.from_str(config.browser_impersonate.lower())`) when `config.browser_impersonate` is set (default: `"chrome"`, env: `KIKUSAN_BROWSER_IMPERSONATE`). Presents a real browser's TLS/HTTP fingerprint via `curl_cffi`, since Google can force-rotate a session's cookies the instant they're used from a different fingerprint than the browser that issued them — this can invalidate a freshly exported, never-reused cookies.txt on its very first use by yt-dlp. `curl-cffi` is pinned to `<0.16.0` in `pyproject.toml` since yt-dlp (as of 2026.7.4) only supports curl_cffi 0.5.10 or 0.10.x-0.15.x; newer versions import-error out of impersonation support entirely (silent "unavailable" in `--list-impersonate-targets`, not an exception, unless the module is imported directly)
  - **JS runtime for signature/n-parameter deciphering**: The Dockerfile installs Deno (yt-dlp's default/recommended EJS challenge-solver runtime) since the base image ships with none, which was silently degrading extraction to whatever pre-signed formats happened to survive YouTube's SABR format-stripping. `remote_components: ["ejs:github"]` in `_get_ydl_opts()` lets yt-dlp fetch the actual solver scripts from GitHub at runtime
...
- `--browser-impersonate`: Browser to impersonate at the network level via curl_cffi (env: `KIKUSAN_BROWSER_IMPERSONATE`, default: `chrome`)
